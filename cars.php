...
if (!empty($_GET['id'])) {
	...
	$categoryId = $_GET['id'];
	$query = "SELECT car.* FROM car INNER JOIN carcategory CC ON CC.CarId = car.Id INNER JOIN category C ON C.Id = CC.CategoryId where C.ID=" . $categoryId . " AND car.Published = 1  order by car.timestamp desc";
